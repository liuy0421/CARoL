...
void loop() {
	...
	for (int i = 0; i < 4; i++) {
		...
		goal.target_pose.pose.position.z = 0.0;
		goal.target_pose.pose.orientation = tf::createQuaternionMsgFromYaw(90 * i);
...
bool isOpen() {
	...
	for (int i = last_scan_in.ranges.size() / 4; i < (3 * last_scan_in.ranges.size()) / 4; i++) {
		std::cout << last_scan_in.ranges[i] << std::endl;
	...
	std::cout << "the real count is " << realCount << std::endl;
	std::cout << "the long count is " << longCount << std::endl;

	if (longCount >= realCount/ 2.0) {
...
bool sayRandomPhraseImpl(ros::Publisher sound_pub, int index, const std::vector<RoomInfo *> &roomInfo){
    ...
    if (isOpen()) {
	messages = std::string("Hello, ").append(cur_name.append(", I hope you had a great Thanksgiving."));
...
void sayRandomPhrase(ros::Publisher sound_pub, int index, const std::vector<RoomInfo *> &roomInfo){
	for (int i = 0; i < 3; i++) {
		if (sayRandomPhraseImpl(sound_pub, index, roomInfo)) {
			break;
		}
	}
}	
